...
function setHiddenSingles(arr: number[][][], sudo: number[][]) {}
function isSolved(sudo: number[][]): Boolean {
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      if (sudo[i][j] == 0) {
        return false
...
function genPossibleCol(sudo: number[][]) {
  let arr: number[][][] = []
  for (let i = 0; i < 9; i++) {
    arr.push([])
    for (let j = 0; j < 9; j++) {
...
// getRelated(0, 4, window.sudo)
interface Related {
  row: number[]
  col: number[]
  grid: number[]
}
function getRelated(i: number, j: number, sudo: number[][]): Related {
...
class Coll {
  ...
  public diff(): any[] {
    ...
    return Array.from(
      new Set(a.concat(b).filter(i => !(aSet.has(i) && bSet.has(i))))
    )
  }
}
